...
const [movin, setMovin] = useState<boolean>(false);
...
// ─────────────────────────────────────────────────────────────────────────────
 const finalizeCurrentWord = useCallback(async () => {
  ...
  if (code.current) {
    ...
    console.log("code.current: " + typeof code.current);
    console.log("dictionary: " + dictionary);
    console.log("dictionary of code.current: " + dictionary[code.current]);
...
const isHittin = useRef<boolean>(false);
...
const handleMouseMove = useCallback((e: MouseEvent) => {
 ...
 timerRef.current = setTimeout(() => {
  ...
  setPosition((prev) => {
    const newX = (400 + e.movementX);
    const newY = (300 + e.movementY);
  ...
  console.log("when moving: " + isHittin.current);


  if (e.movementX > threshold && Math.abs(e.movementY) <= anglethreshold) {
   if (!isHittin.current) {
       // Case 1: movementX > threshold, |movementY| <= threshold
       isHittin.current = true;
       codeChar = sideMappings[1];
   }
} else if (e.movementX > anglethreshold && e.movementY > anglethreshold) {
   if (!isHittin.current) {
       // Case 2: movementX > threshold, movementY > threshold
       isHittin.current = true;
       codeChar = sideMappings[2];
   }
} else if (e.movementX > anglethreshold && e.movementY < -anglethreshold) {
   if (!isHittin.current) {
       // Case 3: movementX > threshold, movementY < -threshold
       isHittin.current = true;
       codeChar = sideMappings[8];
   }
} else if (e.movementX < -threshold && Math.abs(e.movementY) <= anglethreshold) {
   if (!isHittin.current) {
       // Case 4: movementX < -threshold, |movementY| <= threshold
       isHittin.current = true;
       codeChar = sideMappings[5];
   }
} else if (e.movementX < -anglethreshold && e.movementY > anglethreshold) {
   if (!isHittin.current) {
       // Case 5: movementX < -threshold, movementY > threshold
       isHittin.current = true;
       codeChar = sideMappings[4];
   }
} else if (e.movementX < -anglethreshold && e.movementY < -anglethreshold) {
   if (!isHittin.current) {
       // Case 6: movementX < -threshold, movementY < -threshold
       isHittin.current = true;
       codeChar = sideMappings[6];
   }
} else if (Math.abs(e.movementX) <= anglethreshold && e.movementY > threshold) {
   if (!isHittin.current) {
       // Case 7: |movementX| <= threshold, movementY > threshold
       isHittin.current = true;
       codeChar = sideMappings[3];
   }
} else if (Math.abs(e.movementX) <= anglethreshold && e.movementY < -threshold) {
   if (!isHittin.current) {
       // Case 8: |movementX| <= threshold, movementY < -threshold
       isHittin.current = true;
       codeChar = sideMappings[7];
   }
} else {
   isHittin.current = false;
}
  if (codeChar === " ") {
   finalizeCurrentWord();
 } else if (codeChar) {
...
const drawScene = useCallback(() => {
  ...
  ctx.fillStyle = "gray";
  ctx.beginPath();
  ctx.arc(position.x, position.y, 11, 0, 4 * Math.PI);
  ctx.fill();
...
return (
  <div style={{ textAlign: "center", color: "white" }}>
    ...
    <canvas
      ...
      width={800}
      height={600}
      style={{backgroundColor: "lightblue", border: "1px solid white" }}
    />
  </div>
);
};
